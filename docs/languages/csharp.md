...
## 오류와 비동기
- 예외는 실패 상황 표현에 사용한다.
- catch한 예외는 처리 근거를 남기거나 호출자에게 전달한다.
...
## 주석
- XML doc은 프로젝트의 문서 생성·분석기 설정과 기존 관례를 우선한다.
- XML doc이 필요한 선언은 `<summary>`를 간결하게 쓰고, 계약 이해에 필요한 경우에만 `<param>`, `<typeparam>`, `<returns>`, `<exception>` 등을 추가한다.
- 구현 이유와 제약을 설명하는 일반 주석은 `//`를 사용한다.
