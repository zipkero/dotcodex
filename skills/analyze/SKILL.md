...
## 목적
- 구현 전 시스템 동작, 원인, 영향 범위를 파악한다.
- 분석은 실제 코드, 로그, 에러, 문서에 근거해야 한다.

## 원칙
- 파일을 수정하지 않는다.
- 추정은 명시하고, 근거와 분리한다.
- 원인 불명 버그, 다중 파일 영향, API/DB/auth/external integration 변경, 상태/동시성 관련 작업은 문서 우선 플로우 대상인지 판단한다.
- 분석 중 구현이 필요해 보이면 바로 수정하지 말고 필요한 다음 단계를 제안한다.

## 출력 구조
- 요약: 1-2문장 결론
- 근거: 확인한 파일, 로그, 명령, 관찰 사항
- 흐름: 실행 흐름, 데이터 흐름, 상태 변화 중 관련 있는 내용
