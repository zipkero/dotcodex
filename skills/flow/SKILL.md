...
선택된 코드 범위의 실행 흐름, 데이터 흐름, 책임 흐름, 존재 이유를
대화 안에서 설명한다.
...
- 기본은 코드블록 안 텍스트 다이어그램이다.
  터미널과 대화창에서 그대로 읽히는 형태를 우선한다.
- Mermaid는 사용자가 요청했거나 렌더링 가능한 환경이 명확하고,
  복잡한 분기나 상태 전이를 더 잘 설명할 때만 사용한다.
- 호출과 메시지 순서는 들여쓴 흐름 줄, 분기 로직은 tree나 numbered branch,
  상태 전이는 상태 목록과 전이 화살표를 우선한다.
