...
- Task 분해 방식만 여러 가지인 경우에는 한 번의 `verify`로 승인 또는 거절을 판단할 수 있는 경계를 기준으로
  권장 분해안을 작성한다.
...
## 작성 규칙
- Task는 한 번의 `verify`로 평가 가능한 책임 단위로 나누며, 코드 조각 단위로 쪼개지 않는다.
- 사용자 관찰 결과, 실패 의미, 검증 기준이 실제로 달라질 때만 Task를 분리한다.
