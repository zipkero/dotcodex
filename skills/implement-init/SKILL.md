...
- `analysis.md`에 미해결 결정 필요 사항이나 구현 범위, 설계, 검증에 영향을 주는 열린 질문이 있으면 사용자에게 알리고, 사용자가 명시적으로 진행을 원할 때만 계속한다.
...
- `implement.md`는 구현자가 목적, 대상, 접근, 검증 방법을 바로 실행할 수 있게 쓰고, 용어는 `AGENTS.md`의 문서 용어 선택을 따른다.

## 작성 기준
- 설계 결정은 `analysis.md`의 결정 필요 사항에 두고, `implement.md`에는 확정된 설계를 실행하는 Task를 둔다.
- 각 Task는 하나 이상의 `SPEC 완료 조건`과 연결한다.
- 각 Task는 `SPEC 완료 조건`의 의미를 유지하면서 구현 대상, 접근, 검증 조건을 구체화한다.
- `analysis.md`에 없는 설계 판단이 필요한 항목은 `analysis.md 갱신 필요 사항`으로 보고한다.
- 각 Task 이름은 목적, 대상, 접근, 검증 조건을 읽고 실행 범위를 판단할 수 있게 쓴다.
