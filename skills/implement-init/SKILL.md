...
## 작성 규칙
- Task는 외부에서 확인 가능한 결과를 완성하고 한 번의 `verify`로 승인 또는 거절할 수 있는 단위로 묶는다.
- 해당 결과에 필요한 기반·연결·오류 처리는 함께 두고, 내부 구현 순서는 `접근`에 적는다.
- 독립적으로 완료·검증할 수 있고 별도 인도·의존성 해소·위험 격리·검토에 실익이 있을 때 나눈다.
...
- 각 Task는 하나 이상의 적용 중인 `SPEC §5.N`에 매핑하며, 여러 완료 조건을 함께 충족할 수 있다. 전체 Task 집합은 적용 중인 모든 완료 조건을 커버해야 한다.
