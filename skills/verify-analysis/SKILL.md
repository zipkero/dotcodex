...
4. 경계를 가로지르는 인터페이스와 유지해야 할 contract가 빠짐없이 정의됐다.
5. 실제 변경 대상과 원본 조사로 확인한 의존 관계가 영향 범위에 포함됐다.
6. 주요 Decision Point마다 채택안, 근거, 배제한 주요 대안과 trade-off가 다음 단계에서 재결정할 필요 없게 남아 있다.
...
검증 중에는 `analysis.md`, feature 상태판, 코드, 테스트, 설정과 다른 산출물을 수정하지 않는다.

## 출력 구조
1. Status: `approved` 또는 `rejected` 후보
2. Target: 검증한 feature와 `analysis.md`
3. Validation: 판단 기준과 같은 순서로 다음 내용을 적는다.
   - Criterion: 판정할 기준
   - Source: 관련 `SPEC §5.N`, 분석 섹션 또는 프로젝트 contract
   - Evidence: 직접 확인한 문서, 코드, 테스트, 설정 또는 명령 결과
   - Result: `충족` | `불충족` | `근거 부족`
4. Issues: 거절 사유와 다시 필요한 작성 단계인 `analyze-init`; 없으면 `없음`
