...
Task의 중심 메소드는 한 문단으로 압축하지 말고 처리 순서를 단계로 풀어 설명한다.
...
라인 번호를 따라가는 설명은 피한다. 파일 링크와 라인 링크는 근거가 필요한 첫 언급에만 사용하고, 같은 파일 링크를 반복하지 않는다. 같은 파일의 여러 메소드나 테스트를 설명할 때는 첫 파일 링크 뒤에 메소드명 또는 테스트 함수명만 사용한다. 메소드 설명은 파일 설명 안에 섞지 말고 `주요 메소드 흐름`에서 분리하며, 핵심 메소드는 처리 순서를 단계로 풀어 설명한다.
