...
3. Task 하나를 `implement` 기준으로 구현한다. worker가 보고한 결함은 재현 근거·사용자 영향·Task 관련성을 먼저 검토한다.
   현재 또는 앞서 승인된 Task에 영향을 주면 원인에 따라 아래 재시도·정지 규칙으로 구현 수정, 근거 보완, 재검증 또는 사용자 결정에 연결한다.
   두 범위 모두와 독립적인 실패는 근거와 함께 완료 보고에 남긴다.
   worker가 `blocked`를 반환하면 main이 기존 승인과 입력으로 해소할 수 있는지 검토하고, 입력을 실제로 보완했으면 같은 역할에 다시 위임한다.
   해소할 수 없으면 문서와 Task 상태를 유지하고 중단한다. `completed`를 반환한 경우에만 `verify`를 실행한다.
...
- 현재 또는 앞서 승인된 Task와 관련된 결함을 해결하려면 승인 범위 밖 변경이 필요하다.
...
- 수정하지 않은 독립적인 실패와 재현 근거·사용자 영향·Task 관련성
