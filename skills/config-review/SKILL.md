...
## 공식 기준 참조
- Codex의 현재 동작, `AGENTS.md` 구성, 프롬프트·컨텍스트 설계, skill 책임 범위처럼
...
- 문구와 구조 품질
  - 규칙 간 의미 충돌, 모호한 표현, 같은 규칙의 중복을 확인한다.
  - 의미, 조건, 예외, 권한, 중단 기준을 보존하면서 줄일 수 있는 표현을 찾는다.
  - 반복 문장, 원칙을 대신하는 긴 예시, 과한 배경 설명은 압축 후보로 본다.
  - 압축 뒤 실행 판단이나 역할·책임·근거 기준이 달라지면 제안하지 않는다.
  - 목록 항목 하나에 독립적인 규칙이 여러 개 묶였는지 확인한다.
  - 구체적인 예시나 도구 나열이 원칙보다 커져 적용 범위를 좁히는지 확인한다.
  - 의미 없는 부정형 반복, 폐기된 설명, 현재 설정에 없는 기능을 확인한다.
...
- 성능 영향 판단
  ...
  - 영향은 질문 빈도, 중단 시점, 변경 범위, 출력 형식처럼 관찰 가능한 행동으로 설명하고,
    요청 수행을 불필요하게 막거나 반복 중단을 일으킬 때만 성능 문제로 보고한다.
- 변경 제안 판단
