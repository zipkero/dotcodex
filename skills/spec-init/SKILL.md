...
- 사용자의 요청이나 확인된 최종 사용 가능 상태 밖의 목표, 설계, 구현 방식을 새 요구사항으로 추가하지 않는다.
- 질문으로 해소한 요구사항 판단은 대화에만 남기지 않고 `범위`, `제약`, `제외 범위`,
  `완료 조건` 중 맞는 섹션에 반영한다.
- 최종 사용 가능 상태를 판단하되, 입력 근거나 질문으로 확인되지 않은 목표를 요구사항 범위에 추가하지 않는다.
