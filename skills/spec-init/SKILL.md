...
- 대화와 조사에서 확인된 사실, 사용자 판단, assistant의 추정을 구분하고 확인된 사실과 확정된 판단만 요구사항으로 쓴다.
- 사용자가 명시적으로 부정하거나 제외한 항목은 요구사항이 아니라 `제외 범위` 후보로 본다.
- 사용자가 예시로 든 구현 방식, 임시 아이디어, 비교 대상과 assistant의 추정은 사용자가 확정하지 않는 한 요구사항으로 쓰지 않는다.
...
- 공개 규약, 완료 조건, 제외 범위, 후속 문서 기준은 추정으로 확정하지 않는다.
- 이미 확정된 판단은 다시 묻지 않으며, 결과에 영향을 주는 미확정 판단이 남아 있으면 `spec.md`를 생성하지 않는다.
...
- `features/<feature-dir>/README.md`의 `개요`는 기능의 목적과 배경을 1-3문장으로 요약하고, 세부 요구사항이나
  설계 판단은 반복하지 않는다.
- `spec.md`를 다시 작성하면 README의 `[x] SPEC`은 유지하고 `ANALYZE`, `IMPLEMENT`를 `[ ]`로 되돌린다.
