...
- README에서는 현재 동작, 사용 방법, 명령과 문서 경로를 확인한다.
  요구사항의 근거로 삼는 현재 사실은 관련 코드·설정·테스트 또는 실행 결과와 대조한다.
...
## 요구사항 확정
- 현재 요청·같은 작업의 확정 판단·입력 문서·조사 결과에서 확인한 사실과 사용자 결정만 요구사항에 반영한다.
  assistant의 추정과 사용자가 확정하지 않은 구현 예시·임시 아이디어·비교 대상은 제외한다.
...
## 완료 조건 작성 기준
- 완료 조건은 적용 조건과 기대 결과를 명시해 외부에서 충족 여부를 판정할 수 있게 쓴다.
...
- 프로젝트 기준 문서 연결, 요구사항 확정과 완료 조건 작성 기준에 따라 입력과 본문을 대조하고,
  확정된 요구사항·제약의 누락이나 의미 변경이 없는지 확인한다.
