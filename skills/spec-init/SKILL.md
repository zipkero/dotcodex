...
- 요구사항의 출처나 정리 방식이 아니라 사용자가 요청했거나 입력 근거에서 확인되는 최종 사용 가능 상태를
  기준으로 spec 범위를 판단한다.
- 사용자가 실제 서비스, 운영, 프로덕션, 상용 수준처럼 최종 사용 가능 상태를 넓게 암시하는 표현을 사용하면
  이를 단순 MVP나 데모 범위로 축소해 해석하지 않는다.
...
- 질문이 필요한 경우에는 먼저 기존 문서, 로드맵, 코드에서 확인한 사실과 미확정 판단을 분리해
  보고한다.
- 미확정 판단이 공개 contract, 완료 조건, 제외 범위, 후속 문서 기준을 고정하는 경우에는
  그 영향을 설명한 뒤 선택지를 제시하고 권장안을 밝힌다.
...
## 승인 전 확인 기준
- 사용자가 SPEC 승인 전에 답해야 할 feature 고유의 판단 질문을 만들 수 있을 때만 `범위` 앞에
  `승인 전 확인` 섹션을 둔다.
