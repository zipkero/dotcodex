...
- 언어와 프로젝트 성격에 맞는 기본 설계 원칙을 따른다.
  특히 단일 책임, 명확한 경계, 의존성 방향, 공개 contract 보존을 우선하되,
  요청 범위를 넘는 추상화나 리팩터링으로 확장하지 않는다.
- 변경이 책임 분리, 경계, 공개 contract, 상태 소유권, 의존성 방향에 영향을 주면
  영향 범위와 필요한 결정을 먼저 보고한다.
...
- 테스트 통과만을 목적으로 운영 로직에 하드코딩, fixture 전용 특수분기,
  근거 없는 상수값 고정, assertion에 맞춘 동작 축소를 넣지 않는다.
...
- 코드만으로 드러나지 않는 제약, 외부 계약, 예외적 변환, 바꾸면 깨지는 선택은
  가장 가까운 위치에 요지를 짧게 남긴다.
