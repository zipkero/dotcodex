...
- 코드만으로 드러나지 않는 제약, 외부 계약, 예외적 변환, 바꾸면 깨지는 선택은 가장 가까운 위치에 요지를 짧게 남긴다.
- 이름과 코드 흐름만으로 의도나 보장이 충분히 드러나지 않는 함수, helper, 분기, 테스트에는 왜 필요한지와 유지해야 할 조건을 짧게 남긴다.
- 공개 경계에는 호출자가 알아야 하는 책임이나 계약을 짧게 남긴다.
