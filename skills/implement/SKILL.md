...
- 테스트 코드는 명시적 테스트 Task가 있을 때 작성한다.
- 버그 수정의 단일 회귀 테스트가 요청 충족에 직접 필요하면 파일 수정 전에 보고한 뒤 최소 범위로 작성할 수 있다.
- 그 외 Per-Request mode에서는 테스트를 추가하지 않는다. 회귀 위험은 완료 보고의 한계로 남긴다.
