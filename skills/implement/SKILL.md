...
## 주석
- 적용되는 상위 지침과 프로젝트·언어 관례를 따른다.
- 새로 만들거나 변경하는 선언과 구현에는 코드만으로 드러나지 않는 책임, 제약, 판단 근거와 흐름을 설명하는 주석을 작성한다.
- 주석은 구현의 복잡도에 비례해 핵심 의도와 불변 조건을 충분히 설명하고, 코드 자체를 반복하지 않는다.
- 검증에서는 변경된 코드의 주석이 구현과 일치하며 유지보수에 필요한 설명을 빠뜨리지 않았는지 확인한다.
