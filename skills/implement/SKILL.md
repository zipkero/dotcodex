...
- 주석은 이름과 구조로 드러나지 않는 도메인 제약, 순서 의존, 공개 API 책임처럼
  코드와 수명을 같이할 의도가 있을 때만 남긴다.
- 변경 경위, task/Decision 식별자, changelog 내용처럼 구현 과정의 메타 정보는 주석에 넣지 않는다.
- 로직을 수정하면 관련 주석도 함께 갱신하거나 제거한다.
