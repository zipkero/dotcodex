---
name: implement
description: "Execute one documented Task or a small per-request code change within the requested scope."
---

# Implement

## 목적
- Phased mode에서는 `implement.md`의 Task 하나를 최소 범위로 구현한다.
- Per-Request mode에서는 사용자가 요청한 작은 변경을 문서 플로우 없이 처리한다.

## 컨텍스트 로딩
1. Phased mode로 진입하는 경우:
   - 사용자가 구현 의도를 밝히고 `features/<feature-dir>/` 또는 `features/<feature-dir>/implement.md`를 지정했다.
   - feature 경로만 언급한 경우에는 구현하지 않고 요청 의도에 맞춰 분석, 설명, 검토로 처리한다.
2. Phased mode 동작:
   - `spec.md`, `analysis.md`, `implement.md`를 읽는다.
   - 사용자가 `task-<nnn>`을 지정하면 해당 Task를 잡고, 지정하지 않으면 위에서부터 첫 미완료 Task를 잡는다.
   - Task가 없거나 이미 완료되었거나 둘 이상으로 해석되면 구현하지 않고 범위를 요청한다.
3. Per-Request mode:
   - Phased mode 조건이 없고 요청이 작고 명확하며 되돌리기 쉬운 변경이면 바로 처리한다.
   - 작고 되돌리기 쉬운 변경이라도 사용자의 의도, 산출물, 변경 범위, 성공 기준처럼 요구사항 기준이
     여러 방향으로 해석될 수 있으면 먼저 질문한다.
   - 질문이 필요한 상태에서는 파일을 수정하지 않는다.
   - 조사로 해소 가능한 불확실성은 먼저 코드, 테스트, 로그, 기존 문서에서 확인한다.
   - `features/<feature-dir>/`를 만들지 않는다.

## 범위와 설계 변경
- 요청 또는 Task 범위 안의 구현 선택은 기존 코드 패턴과 `analysis.md`의 확정된 결정을 따른다.
- Task 밖에서 발견한 문제는 수정하지 않고 보고만 한다.
- 구현이나 테스트 완료에 새 의존성, 인터페이스, 추상화, 공개 API, 경계, 상태 소유권, 의존성 방향,
  인접 리팩터링 또는 확정된 설계 변경이 필요하면 파일 수정 전에 중단한다.
- 중단 시 필요한 이유, 영향, 대안을 보고하고 사용자 결정을 요청한다.
  Phased mode의 설계 변경은 `analysis.md` 갱신 필요 사항도 함께 보고한다.

## 구현 규칙
- 기존 코드 패턴과 프로젝트 관례를 우선한다.
- 언어와 프로젝트 성격에 맞는 기본 설계 원칙을 따른다. 특히 단일 책임, 명확한 경계, 의존성 방향, 공개 contract 보존을 우선하되,
  요청 범위를 넘는 추상화나 리팩터링으로 확장하지 않는다.
- 새로 만들거나 바꾸는 이름은 현재 역할과 책임을 기준으로 붙이고, 같은 개념을 가리키는 타입, 필드, 메서드, 주석, 테스트 설명은
  일관된 용어 체계를 사용한다.
- 외부 스키마, 프로토콜, 사용자 입력에서 온 이름은 경계에서 보존하고, 내부 이름은 도메인 내 역할에 맞게 표현한다.
- Phased mode에서는 `analysis.md`에 확정된 설계를 해당 언어와 프로젝트 관례에 맞춰 구현한다.
- 파일 수정 전 어떤 변경을 할지 짧게 설명한다.
- 구현은 대상 Task 단위로 진행하며, 기본적으로 한 번에 하나의 Task를 구현한다.
- 사용자가 여러 Task 또는 전체 구현을 명시적으로 요청하면, 각 Task의 범위와 검증 조건을 유지한 채 순서대로 구현할 수 있다.
- 잡은 Task가 닫힌 동작이 아니라 코드 조각 수준이면, 구현하지 말고 Task 재분해 필요성을 보고한다.
- Phased mode의 새 테스트 코드는 Task의 `접근`이나 `검증 조건`에 테스트 작성 범위가 명시된 경우에 작성한다.
- Per-Request mode에서는 요청한 동작을 직접 검증하거나 수정한 결함의 재발을 막는 데 필요한 최소 테스트를
  변경 범위에 포함할 수 있다.
- 새 테스트 기반, 의존성, 공용 fixture 또는 요청 밖 동작의 검증이 필요하면 `범위와 설계 변경` 기준을 따른다.
- 요청 범위 안에서 테스트를 추가하지 못했거나 기존 테스트가 부족하면 완료 보고에 검증 한계와 남은 위험을 적는다.
- 테스트 실패를 해결할 때도 테스트가 기대하는 내부 구조에 운영 코드를 맞추지 말고, 외부 관찰 가능한 동작, 공개 contract,
  `analysis.md`의 설계 결정을 기준으로 원인을 판단한다.
- 테스트 통과만을 목적으로 운영 로직에 하드코딩, fixture 전용 특수분기, 근거 없는 상수값 고정, 검증 코드에 맞춘 동작 축소를 넣지 않는다.
  테스트와 구현이 충돌하면 완료 조건과 공개 contract 기준으로 어느 쪽이 틀렸는지 먼저 판단한다.
- 테스트, 포맷, 빌드 명령은 변경 범위를 확인하는 데 필요한 수준으로 실행한다.
- 검증 단계에서 `approved`로 판단하기 전에는 `implement.md` 체크박스와 `features/<feature-dir>/README.md`의 `IMPLEMENT` 상태를 변경하지 않는다.

## 주석 작성 기준
- 코드만으로 충분히 드러나지 않는 역할, 책임, 이유, 제약과 계약은 설명 대상과 가장 가까운 위치에 주석으로 남긴다.
- 공개 경계와 잘못 이해하면 결함으로 이어지는 동작에는 호출자가 알아야 하는 책임, 보장, 오류 조건과 사용 조건을 설명한다.
- 테스트와 테스트 보조 코드의 이름, 구조와 검증 코드만으로 보장하려는 동작, 계약, 회귀 조건이나 전제가 드러나지 않으면
  주석으로 설명한다.
- 단순 유틸이나 얇은 위임처럼 이름과 구조만으로 역할이 충분히 드러나는 코드는 주석을 생략할 수 있다.
- 코드가 그대로 보여주는 절차, 대입, 호출 순서, 분기 조건과 검증 기대값은 주석으로 반복하지 않는다.
- 여러 심볼에 공통되는 배경은 반복하지 않고 가장 좁은 공통 경계에 한 번만 설명한다.
- 변경 경위, 구현 순서, 계획 식별자처럼 작업 과정에서만 의미가 있는 정보는 주석으로 남기지 않는다.
- 코드와 직접 대응되는 용어를 유지한다.
- 주석의 언어, 문체, 구조, 형식과 배치는 파일, 언어와 프로젝트의 기존 관례를 따른다.
  관례가 없고 사용자가 다르게 요청하지 않았다면 자연스러운 한국어로 작성한다.
- 로직을 수정하면 영향을 받는 기존 주석도 같은 작업에서 갱신하거나 제거한다.

## 완료 보고
- 수행한 작업과 결과
- 변경한 파일
- 실행한 확인과 결과
- 남은 작업 또는 위험
- 구조나 동작을 이해하는 데 필요한 배경이 있으면 변경 전후의 흐름과 지켜야 할 조건도 설명한다.
- Phased mode에서 실제 구현이 Task의 `접근`과 달라졌다면 차이의 성격, 관련 `SPEC §5.N` /
  `ANALYSIS §X.Y`, 문서 반영 여부와 그 근거를 보고한다.
- `analysis.md`, Task의 `목적`, `검증 조건`, `참조`에 영향이 없는 구현 상세 차이만 `접근`에 반영한다.
  그 밖의 차이는 문서를 바꾸지 않고 재분석이나 재작성이 필요하다고 보고한다.
- 구현 전체를 요청받았다면 `verify`까지 이어서 진행하고, 구현 단계만 요청받았다면 검증이 남았음을 알린다.
